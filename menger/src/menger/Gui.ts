...
export class GUI implements IGUI {
  ...
  public drag(mouse: MouseEvent): void {
    ...
    if (mouse.buttons == 1) {
      ...
      let newUp = new Vec3([0.0, y_change, 0.0]);
      let newRight = new Vec3([x_change, 0.0, 0.0]);
      let newStuff = newUp.add(newRight);
      let axis = Vec3.cross(newStuff, this.camera.forward());
      ...
      if (mouse.screenY > this.prevY) {
        let change = this.camera.forward().normalize().scale(GUI.zoomSpeed);
        this.camera.setPos(this.camera.pos().add(change));
        this.camera.setTarget(this.camera.target().add(change));
      }
